...
function loadRecords() {
  try { return JSON.parse(localStorage.getItem(LS_POOP) || '[]') } catch { return [] }
}
...
function removeRecord(id) {
  try {
    localStorage.setItem(LS_POOP, JSON.stringify(loadRecords().filter(r => r.id !== id)))
  } catch {}
}
...
// ── PoopEntryCard ──────────────────────────────────────────
function PoopEntryCard({ record, onUpdate, onDelete }) {
  ...
  return (
    <div className="bg-white rounded-xl overflow-hidden">
      ...
      <div className="px-4 pt-1 pb-3 border-b border-black/15">
        <p className="text-xs font-normal text-text-placeholder mb-2">うんちメモ</p>
        <textarea
          value={record.memo || ''}
...
// ── CatPoopScreen ──────────────────────────────────────────
export default function CatPoopScreen({ cat, date, onBack }) {
  ...
  return (
    <div className="flex flex-col h-dvh bg-[#F7F7F7] overflow-hidden">
      ...
      <header className="relative flex items-center justify-center h-[60px] px-4 bg-white border-b border-black/10 flex-shrink-0">
        ...
        <span className="text-base font-semibold text-text-primary">うんち</span>
      </header>

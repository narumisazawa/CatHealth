...
const PRESET_VALUES = ['良好', '軟便', '下痢', '便秘', '血便']
...
// ── DailyPoopSheet ────────────────────────────────────────
export default function DailyPoopSheet({
  catId,
  selectedDate,
  onClose,
  onSave,
  onDelete,
  initialRecord = null,    // null = 新規、object = 編集
}) {
  ...
  // 編集時：保存値がプリセット外なら「その他」扱い
  const savedCondition = initialRecord?.condition ?? '良好'
  ...
  function handleSave() {
    ...
    if (!savedCond) return  // その他選択で未入力なら保存しない
    const record = {
    ...
    try {
      const prev = JSON.parse(localStorage.getItem(LS_POOP) || '[]')
      const next = isEdit
        ? prev.map(r => r.id === record.id ? record : r)
        : [...prev, record]
      localStorage.setItem(LS_POOP, JSON.stringify(next))
    } catch { /* ignore */ }
    onSave(record)
  }
  ...
  function handleDelete() {
    ...
    try {
      const prev = JSON.parse(localStorage.getItem(LS_POOP) || '[]')
      localStorage.setItem(LS_POOP, JSON.stringify(prev.filter(r => r.id !== initialRecord.id)))
    } catch { /* ignore */ }
        ...
        <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 40 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, padding: '8px 16px 0' }}>
            ...
            <div style={{ padding: '4px 4px 2px' }}>
              <span style={{ fontSize: 12, color: 'rgba(0,0,0,0.7)' }}>うんち確認</span>
            </div>
            ...
            <div style={{ background: '#FFFFFF', borderRadius: 12, overflow: 'hidden' }}>
              ...
              <div style={{ position: 'relative' }}>
                <div style={{
                  ...
                  <span style={{ fontSize: 14, color: '#111827', flexShrink: 0 }}>状態</span>
            ...
            <div style={{ background: '#FFFFFF', borderRadius: 12, padding: 16 }}>
              ...
              >
                {photo
                  ? <img src={photo} alt="poop" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  : <CameraIcon />
                }
              </button>
            </div>

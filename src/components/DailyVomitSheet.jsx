...
const CONDITION_OPTIONS = [
  { value: '毛玉',           label: '毛玉' },
  { value: '未消化フード',   label: '未消化フード' },
  { value: '液体のみ',       label: '液体のみ' },
  { value: '黄色い液体',     label: '黄色い液体' },
  { value: '血が混じる',     label: '血が混じる' },
  { value: 'other',          label: 'その他（自由入力）' },
]

const PRESET_VALUES = ['毛玉', '未消化フード', '液体のみ', '黄色い液体', '血が混じる']
...
// ── DailyVomitSheet ───────────────────────────────────────
export default function DailyVomitSheet({
  catId,
  selectedDate,
  onClose,
  onSave,
  ...
  const savedCondition = initialRecord?.condition ?? '毛玉'
  ...
  function handleSave() {
    ...
    try {
      const prev = JSON.parse(localStorage.getItem(LS_VOMIT) || '[]')
      const next = isEdit
  ...
  function handleDelete() {
    ...
    try {
      const prev = JSON.parse(localStorage.getItem(LS_VOMIT) || '[]')
      localStorage.setItem(LS_VOMIT, JSON.stringify(prev.filter(r => r.id !== initialRecord.id)))
    } catch { /* ignore */ }
        ...
        <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 40 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, padding: '8px 16px 0' }}>
            ...
            <div style={{ padding: '4px 4px 2px' }}>
              <span style={{ fontSize: 12, color: 'rgba(0,0,0,0.7)' }}>ゲロ確認</span>
            </div>
            ...
            <div style={{ background: '#FFFFFF', borderRadius: 12, padding: 16 }}>
              ...
              >
                {photo
                  ? <img src={photo} alt="vomit" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  : <CameraIcon />
                }
              </button>
            </div>

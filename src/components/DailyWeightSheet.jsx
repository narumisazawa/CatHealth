...
// ── DailyWeightSheet ──────────────────────────────────────
export default function DailyWeightSheet({
  catId,
  selectedDate,
  onClose,
  onSave,
  ...
  const [weight, setWeight] = useState(initialRecord?.weight != null ? String(initialRecord.weight) : '')
  ...
  function handleSave() {
    ...
    try {
      const prev = JSON.parse(localStorage.getItem(LS_WEIGHT) || '[]')
      const next = isEdit
        ? prev.map(r => r.id === record.id ? record : r)
        : [...prev, record]
      localStorage.setItem(LS_WEIGHT, JSON.stringify(next))
    } catch { /* ignore */ }
    onSave(record)
  }
  ...
  function handleDelete() {
    ...
    try {
      const prev = JSON.parse(localStorage.getItem(LS_WEIGHT) || '[]')
      localStorage.setItem(LS_WEIGHT, JSON.stringify(prev.filter(r => r.id !== initialRecord.id)))
    } catch { /* ignore */ }
        ...
        <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 40 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, padding: '8px 16px 0' }}>
            ...
            <div style={{ background: '#FFFFFF', borderRadius: 12, padding: 16 }}>
              ...
              >
                {photo
                  ? <img src={photo} alt="weight" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  : <CameraIcon />
                }
              </button>
            </div>
